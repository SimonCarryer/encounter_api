...
class EncounterSource:
    def __init__(self,
        ...
        if xp_budget is not None:
            ...
            self.xp_budget = self.budget_from_character_dict(
        ...
        if monster_sets is None:
            monster_set = self.random_state.choice(
                self.monster_manual.monster_set_names)
        else:
        ...
        encounters = EncounterBuilder(
            self.xp_budget, monsters, n_characters=self.n_characters).monster_lists
        self.encounter_picker = EncounterPicker(
            encounters, self.xp_budget, n_characters=self.n_characters, random_state=self.random_state)
        self.used_signs = set()
    ...
    def get_encounter(self, difficulty=None, occurrence=None, style=None):
        encounter = self.encounter_picker.pick_encounter(
            difficulty=difficulty, occurrence=occurrence, style=style)
        return self.format_encounter(encounter)
    ...
    def format_encounter(self, encounter):
        ...
        if encounter['monsters'] == []:
            ...
            response['monsters'] = [{'name': k, 'number': v} for k, v in dict(
                Counter([monster['Name'] for monster in encounter['monsters']])).items()]
            response['difficulty'] = encounter['difficulty']
            response['xp_value'] = int(encounter['xp_value'])
            response['raw_xp_value'] = sum(
                [m["XP"] for m in encounter["monsters"]])
            response['monster_hash'] = encounter['monster_hash']
            response['treasure'] = self.get_treasure(encounter['monsters'])
        return response

    def get_sign(self):
        signs = [sign for sign in monster_manual.get_signs(
            self.monster_set) if sign not in self.used_signs]
        if len(signs) > 0:
