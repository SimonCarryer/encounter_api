...
encounter_set_parser.add_argument("monster_set", required=True, help="Monster set.")
...
class EncounterSet(Resource):
    ...
    def get(self):
        ...
        if not all([level <= 20 for level in character_level_dict.keys()]):
            raise BadRequest("Maximum character level is 20")
        source = EncounterSource(
            character_level_dict=character_level_dict, monster_sets=[monster_set]
        )
        encounters = {}
        encounters["basic"] = []
        encounters["exotic"] = []
        ...
        for difficulty in ["easy", "medium", "medium", "hard"]:
            for style in ["basic", "exotic", "leader"]:
                encounter = source.get_encounter(difficulty=difficulty, style=style)
